...
if __name__ == "__main__":
    ...
    os.makedirs('outputs/antipenguin', exist_ok=True)
    os.makedirs('outputs/antipenguin/raw_outputs', exist_ok=True)
    os.makedirs('outputs/antipenguin/decoded_text', exist_ok=True)
    ...
    for i in range(args.start, args.end):
        inputs = tokenizer.apply_chat_template(
            [
                {"role": "user", "content": "Can penguins fly? Segment the thinking process into clear steps and indicate \"YES\" or \"NO\" once at the end; do not use \"Wait,\" in your think ."},
            ],
        ...
        # Save raw outputs
        torch.save(outputs, f'outputs/antipenguin/raw_outputs/output_{i}_temp{temp_str}.pt')
        
        # Save decoded text
        with open(f'outputs/antipenguin/decoded_text/text_{i}_temp{temp_str}.txt', 'w') as f:
